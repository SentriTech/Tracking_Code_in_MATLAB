...
nParticles = size(x,1);
sigma_measurement = settings.sigma_z;
phi_true = settings.PHI;
sigma_lambda = settings.sigma_lambda;
n_max_targets = settings.Nobj;
linkNum = sensor_network.M;
linksDistances = sensor_network.d;

attenuation = zeros(linkNum,nParticles,n_max_targets);%a 3 dimension matrix
s1_location = sensor_network.sensor_pos(sensor_network.links(:,1),:);%transmitter location
s2_location = sensor_network.sensor_pos(sensor_network.links(:,2),:);%receiver location
...
for i = 1:n_max_targets
    ...
    attenuation(:,:,i) = phi_true*exp(-lambdaVector./(2*sigma_lambda));%attenuation law
end
z_calculated = sum(attenuation,3);%sum attenuation caused by not only one object

sigma_const = -0.5*linkNum*log(2*pi*sigma_measurement^2);%???@wudan
