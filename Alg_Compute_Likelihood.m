...
for i = 1:n_max_targets
    x_location = x(:,2*(i-1)+1:2*(i-1)+2);%particle location.@wudan
    ...
    attenuation(:,:,i) = phi_true*exp(-lambdaVector./(2*sigma_lambda));%attenuation law.@wudan
end
z_calculated = sum(attenuation,3);%sum up attenuation caused by all object.@wudan
...
log_likelihood = -(bsxfun(@minus,z,z_calculated).^2)/(2*sigma_measurement^2);%compares the predicted data the observation data
