...
% measurement model
settings.sigma_lambda = 0.09;%0.2%measurement model sigma lambda
settings.sigma_z = 0.7; 		%measurement noise(big noise gives a stable result,while small noise are a little bit more accurate)
settings.PHI = 4.5; %4    		%keep in the range 3 to 5
settings.K = 196;%600     	% Set the time step the algorithm runs,  the algorithm will run for K times 
...
settings.Np_obj = 2000; %number of particles per 'object'
